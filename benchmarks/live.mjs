...
// Retain visible evidence and usage, not opaque provider reasoning payloads.
const encode = value => JSON.stringify(value,(key,value)=>key==='content'&&Array.isArray(value)?value.filter(part=>part.type!=='thinking'):['thinkingSignature','textSignature','responseId','diagnostics'].includes(key)?undefined:value,2);
...
for(const c of selected){
 ...
 const f=await fixture(terminal);let p=rpc(f,{live:true,extension}),target,observe,heldMs=0;const previousEvents=[],previousMessages=[];
 ...
 try{
  ...
  const effect=async()=>{
   switch(c.id){
    ...
    case 'A06':return (await f.api('pane','get',target)).pane.label==='notes';
    case 'A07':{const l=await layout();return l.zoomed&&l.focused_pane_id===target;}
    ...
    case 'S09':return (await f.api('pane','get',target)).pane.label===['copy work','team swarm','roles and workers'][variant];
  ...
  try{
   ...
   if(['S04','S05','S06','S08'].includes(c.id)){
    ...
    if(c.id==='S05'){previousEvents.push(...p.events.slice(offset));previousMessages.push(...p.messages);offset=0;await p.close();p=rpc(f,{live:true,extension});await p.request('get_state');}
   ...
   if(c.id.startsWith('S')&&c.id!=='S09'){
    ...
    if(c.id==='S10'){await wait(()=>received().flatMap(m=>m.details?.ids||[]).length>=2);await wait(async()=>!(await p.request('get_state')).isStreaming);assert.ok(text().length>0);}
    else if(c.id==='S03')await wait(()=>text().includes(marker)&&text().includes(marker2)&&(received().flatMap(m=>m.details?.ids||[]).length>=2));
 ...
 }catch(e){const blocked=/authentication|token is expired|unauthorized|model is not supported/i.test(e.message);if(blocked)authError=e.message;results.push({id:c.id,intent:c.intent,status:blocked?'blocked':'fail',reason:e.message,prompt:c.prompts[variant],elapsedMs:start?Date.now()-start:undefined,conversation:[...previousMessages,...p.messages]});}
 finally{
  const helpers=[];
  for(const name of await readdir(join(f.dir,'parent.jsonl.tmax')).catch(()=>[]))if(name.endsWith('.jsonl')){const entries=(await readFile(join(f.dir,'parent.jsonl.tmax',name),'utf8')).trim().split('\n').flatMap(line=>{try{return [JSON.parse(line)];}catch{return [];}});helpers.push({session:name,messages:entries.filter(e=>e.type==='message').map(e=>e.message)});}
  results.at(-1).helpers=helpers;
  await p.close();await f.close();
 }
