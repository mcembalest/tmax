...
For the actual use case—open, recognize one digit, and close—**batch 1 is the relevant row**. It favors NumPy (0.00496 ms), followed by PyTorch CPU (0.00698 ms); the difference in the kernel itself is tiny compared with process startup, model loading, image preprocessing, and UI teardown. The recorded `setup_seconds` is not a complete cold open measurement, and must not be treated as one.
